...
impl From<serde_json::Error> for MessagingError {
    fn from(err: serde_json::Error) -> MessagingError {
        MessagingError::SerializationError(err)
    }
}

type Result<T> = result::Result<T, MessagingError>;

type Code = isize;
type ID = u64;
type URI = String;
type List = Vec<serde_json::Value>;
type Dict = BTreeMap<String, serde_json::Value>;
type Args = Vec<serde_json::Value>;
type KwArgs = BTreeMap<String, serde_json::Value>;
type Arguments = Option<(Args, Option<KwArgs>)>;
...
pub enum Message {
    ...
    ABORT        (Dict, URI),
    GOODBYE      (Dict, URI),
    ERROR        (MessageErrorCode, ID, Dict, URI, Arguments),
    PUBLISH      (ID, Dict, URI, Arguments),
    PUBLISHED    (ID, ID),
    SUBSCRIBE    (ID, Dict, URI),
    SUBSCRIBED   (ID, ID),
    UNSUBSCRIBE  (ID, ID),
    UNSUBSCRIBED (ID),
    EVENT        (ID, ID, Dict, Arguments),
    CALL         (ID, Dict, URI, Arguments),
    RESULT       (ID, Dict, Arguments),
    REGISTER     (ID, Dict, URI),
    REGISTERED   (ID, ID),
    UNREGISTER   (ID, ID),
    UNREGISTERED (ID),
    INVOCATION   (ID, ID, Dict, Arguments),
    YIELD        (ID, Dict, Arguments),
}
...
pub trait WampMsg {
    ...
    fn from_json_string(msg_str: &String) -> Result<Message>;
}

impl WampMsg for Message {
    fn to_json_string(&self) -> Result<String>
    {
        use self::Message::*;
        use self::MessageCode as C;
